...
import static DataStruct.Ninth.HeapSort.heapsort;
import static DataStruct.Ninth.MergeSort.mergesort;
import static DataStruct.Ninth.RadixSort.radixsort;
...
public class test {
    ...
    private static void timing(int[] arr) {
        ...
        System.out.println("heapsort\t" + (end - start) / 1000);
        a = Arrays.copyOf(arr, arr.length);

        start = System.currentTimeMillis();
        mergesort(a);
        end = System.currentTimeMillis();
        System.out.println("mergesort\t" + (end - start) / 1000);
        a = Arrays.copyOf(arr, arr.length);

        start = System.currentTimeMillis();
        radixsort(a);
        end = System.currentTimeMillis();
        System.out.println("radixsort\t" + (end - start) / 1000);
        a = Arrays.copyOf(arr, arr.length);
